			...
			<form action="${url}/manager/memberDelOK" method="post" id="memberFrm">
				...
				<ul class="memberList_wrap">
					...
					<c:forEach var="vo" items="${list}">
						...
						<li>${vo.tel }</li>
						<li>
							<c:if test="${empty vo.email}">&nbsp;</c:if>
							${vo.email }
						</li>
						<li>
							<c:if test="${empty vo.info}">&nbsp;</c:if>
							${vo.info}
						</li>
						<li>
							<c:if test="${vo.gender == 0}">&nbsp;</c:if>
							<c:if test="${vo.gender == 1 }">남자</c:if>
							<c:if test="${vo.gender == 2 }">여자</c:if>
						</li>
						<li>
							<c:if test="${vo.age == 0}">&nbsp;</c:if>
							<c:if test="${vo.age == 1}">10대</c:if>
							<c:if test="${vo.age == 2}">20대</c:if>
							<c:if test="${vo.age == 3}">30대</c:if>
							<c:if test="${vo.age == 4}">40대</c:if>
							<c:if test="${vo.age == 5}">50대</c:if>
						</li>
						<li>${vo.joindate}</li>
					</c:forEach>
				</ul>
